...
const SCRIPT_PATH = fileURLToPath(
  new URL("../validate-playwright-smoke-manifest.mjs", import.meta.url),
);
const FIXTURES_ROOT = fileURLToPath(
  new URL("fixtures/playwright-smoke-manifest", import.meta.url),
);
...
test("passes when every named critical smoke command discovers tests", () => {
  ...
  assert.match(
    result.stdout,
    /qa:smoke:ui: 1 test in 1 file \(tests\/valid\.spec\.mjs\)/,
  );
  assert.match(
    result.stdout,
