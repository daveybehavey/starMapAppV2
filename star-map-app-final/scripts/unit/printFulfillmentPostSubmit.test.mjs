...
test("already-sent pending retry rereviews same printfulOrderId with zero create/confirm side effects", async () => {
  const printfulOrderId = 98765;
  const pending = baseRecord({
    printfulOrderId,
    printfulFileReviewPendingAt: 1_700_000_000_222,
  });
  const reviewedOrderIds = [];
  const sideEffects = { create: 0, confirm: 0 };

  assert.equal(shouldRereviewPrintfulFilesOnAlreadySent(pending), true);

  const result = await applyAlreadySentRetryReview(pending, {
    maxAttempts: 2,
    retryDelaysMs: [0],
    sleep: async () => {},
    loadStoredPrintOrder: async () => pending,
    reviewPrintfulOrderFiles: async (orderId) => {
      reviewedOrderIds.push(orderId);
      return buildPrintfulOrderFileReview(orderId, [
  ...
  assert.deepEqual(reviewedOrderIds, [printfulOrderId, printfulOrderId]);
  assert.equal(result.printfulOrderId, printfulOrderId);
  assert.equal(isPrintfulFileReviewPending(result), true);
  assert.equal(sideEffects.create, 0);
  assert.equal(sideEffects.confirm, 0);

  // Admin retry gates rereview before any Printful create call site.
  const rereviewCallIdx = retrySrc.indexOf("shouldRereviewPrintfulFilesOnAlreadySent(existing)");
  const submitCallIdx = retrySrc.indexOf("await submitPrintfulOrder(");
  assert.ok(rereviewCallIdx >= 0);
  assert.ok(submitCallIdx > rereviewCallIdx);
  assert.match(retrySrc, /applyPrintfulPostSubmitReview\(existing\)/);
  assert.equal(/\/orders\/[^"'\s]+\/confirm/.test(retrySrc), false);
  assert.equal(postSubmitSrc.includes("submitPrintfulOrder"), false);
});
