...
test("collectPrintfulFileFailures only includes confirmed failed (not waiting)", () => {
  ...
  assert.deepEqual(collectPrintfulFileFailures(rows), [{ item: "c", type: "default", status: "failed" }]);
...
test("summarizePrintfulFileReview: waiting/unknown/empty/null stay pending or unavailable", () => {
  ...
  assert.equal(
    summarizePrintfulFileReview(buildReviewFromStatuses([{ item: "a", type: "default", status: "waiting" }])),
    "pending",
  );
  assert.equal(
    summarizePrintfulFileReview(buildReviewFromStatuses([{ item: "a", type: "default", status: "unknown" }])),
    "pending",
  );
  assert.equal(
    summarizePrintfulFileReview(buildReviewFromStatuses([{ item: "a", type: "default", status: "ok" }])),
    "healthy",
  );
  assert.equal(
    summarizePrintfulFileReview(buildReviewFromStatuses([{ item: "a", type: "default", status: "failed" }])),
    "failed",
  );
});
