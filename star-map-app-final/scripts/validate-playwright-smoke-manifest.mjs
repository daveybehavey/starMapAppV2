...
function discover(root, suite) {
  ...
  const result = spawnSync(
    npm,
    ["run", "--silent", suite.command, "--", "--list", "--reporter=line"],
    {
      cwd: root,
      encoding: "utf8",
      env: {
  ...
  const totals = [
    ...output.matchAll(/Total:\s+(\d+)\s+tests?\s+in\s+(\d+)\s+files?/g),
  ];
...
function validateManifest(root, manifest, pkg) {
  ...
  if (!sameArray(commandNames, REQUIRED_COMMANDS)) {
    errors.push(
      `${MANIFEST_FILE} critical commands must be exactly: ${REQUIRED_COMMANDS.join(", ")}`,
    );
  }
  ...
  for (const suite of manifest.criticalSuites) {
    if (
      ...
      errors.push(
        `${MANIFEST_FILE} has an invalid critical suite entry: ${JSON.stringify(suite)}`,
      );
      continue;
    }
  ...
  if (Array.isArray(manifest.exemptions)) {
    for (const exemption of manifest.exemptions) {
      if (
        ...
        errors.push(
          `${MANIFEST_FILE} exemption entries require non-empty "spec" and "reason" strings`,
        );
        continue;
      }
      if (!fs.existsSync(path.join(root, exemption.spec))) {
        errors.push(
          `${MANIFEST_FILE} exemption references missing spec "${exemption.spec}"`,
        );
      }
      if (selected.has(exemption.spec)) {
        errors.push(
          `${MANIFEST_FILE} spec "${exemption.spec}" cannot be both critical and exempt`,
        );
      }
    }
  }
