...
| **QA exclusion**     | Same markers as `commerceAnalyticsQa.mjs` (`qa_run`, `qa_ops_checkout`, `qa_source` prefixes `live_conversion*` / `live_print_conversion*`). Legacy sessions whose only marker is `client_reference_id=qa-live-conversion` must be classified as QA **before** that identifier is stripped — either exclude the row, set a non-identifying QA flag, or let the CLI derive authoritative `qa_source=live_conversion_legacy` from the exact marker (even if another unrecognized `qa_source` is present; never retain/print the raw client-reference). Excluded counts/revenue reported separately. |
...
| `plan`                                     | digital\* / print | Digital: `single` \| `pack3` \| `subscription` (missing/blank/unsupported → unknown; never default to single). Print: omit or `single` only — any other nonblank plan (e.g. `pack3`/`subscription`) is rejected at sanitize, even with `include_digital`.                                                                                                                                                                                                                                                                                                                 |
| `print_variant`                            | print             | catalog id (`poster_framed`, …)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| `include_digital`                          | no                | Strict boolean — print + HD digital bundle. Accepted forms only: JSON booleans `true`/`false`; integers `0`/`1`; strings `true`/`false`/`1`/`0`/`yes`/`no` (trimmed, case-insensitive). Stripe checkout alias `print_include_digital` (and camel/Pascal forms) is accepted and normalized to this field before unknown-field stripping. Malformed values (e.g. `"tru"`, `2`, objects, arrays, null) and conflicting canonical+alias values fail closed.                                                                                                                   |
| `include_card`                             | no                | Strict boolean — print + card bundle; **only** valid when `order_type=print`, `print_variant=poster_framed`, and `include_digital=false` (checkout semantics). Same accepted forms and Stripe alias `print_include_card` (camel/Pascal) as `include_digital`. Any other combination or malformed value is rejected at sanitize.                                                                                                                                                                                                                                           |
| `shipping_country`                         | print\*           | ISO-2; required to resolve shipping cost. Stripe checkout alias `print_shipping_country` (and camel/Pascal forms) is accepted and normalized to this field before unknown-field stripping. Matching canonical+alias values succeed; conflicting values fail closed.                                                                                                                                                                                                                                                                                                       |
| `shipping_charge_cents`                    | no                | customer shipping charged (informational; `amount_total` already net). Stripe checkout alias `print_shipping_charge_cents` (and camel/Pascal forms) is accepted and normalized to this field before sensitive/unknown-field stripping. Matching canonical+alias values succeed; conflicting values fail closed. Same non-negative integer minor-unit validation as the canonical field.                                                                                                                                                                                   |
| `shipping_subsidy_cents`                   | no                | waived shipping (informational; do not double-count). Stripe checkout alias `print_shipping_subsidy_cents` (and camel/Pascal forms) is accepted the same way as `shipping_charge_cents`.                                                                                                                                                                                                                                                                                                                                                                                  |
| `discount_cents`                           | no                | reported discount (informational; `amount_total` already net)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| `qa_run` / `qa_ops_checkout` / `qa_source` | no                | QA exclusion markers. Prefer these over identifiers. For legacy QA whose only Stripe marker was `client_reference_id=qa-live-conversion`, set a non-identifying flag (e.g. `qa_source` starting with `live_conversion`) **before** stripping the identifier — or exclude the row. The CLI also derives `qa_source=live_conversion_legacy` from that exact marker during sanitize; that derived marker is **authoritative** even if the row also has an unrecognized nonempty `qa_source` (e.g. `manual`). The raw client-reference is never kept on the sanitized record. |
...
3. **Legacy QA before identifier removal:** If a session’s only QA marker is `client_reference_id=qa-live-conversion`, either **exclude the row** from the export or **derive a non-identifying QA flag** (`qa_run` / `qa_ops_checkout` / `qa_source` with a `live_conversion*` prefix) **before** stripping `client_reference_id`. Never leave the raw client-reference value in the sanitized file, logs, or fixtures. (If the exact legacy marker is still present at CLI ingest, the CLI classifies it as QA, sets authoritative `qa_source=live_conversion_legacy` even when another nonempty unrecognized `qa_source` is present, drops the identifier from the in-memory record, and excludes the row — still never printing or persisting the raw value.)
4. **Strip** session IDs, customer objects, emails, names, phones, addresses, payment intents/methods, `client_reference_id`, `print_asset_id`, `print_card_asset_id`, `map_id`, raw `metadata` blobs, and any tokens/secrets.
5. **Exclude** merch checkouts (`print_merch_*` present) until a dedicated merch contribution model exists — do not map them as poster/canvas rows.
6. Save as `schema_version: 1` JSON. Never commit production exports.

The CLI **rejects** sensitive/row-identifying fields (other than the exact legacy QA client-reference marker, which is consumed into a non-identifying flag) and unsupported merch markers with a nonzero exit and never prints secret/PII/asset values.
...
| **Recovery failure**                                    | Recovery email send error or customer cannot regain access                                                                  | Per-invocation attempt KV `stripe:checkout_recovery:attempt:<sessionId>:<attemptId>` (sanitized `errorCode` / `retryability` / `status` only; no raw bodies or customer email); delivered marker + session `recoveryEmailSentAt` only after success; historical session failure fields may exist on older records but are not written for new failures; success/download recovery PostHog events; support paths | `partial`                             | medium for send-state taxonomy; low–medium for regain-access outcomes           |
...
16. **Funnel CLI reconcile script** — restored in #184 (`qa:funnel-reconcile` dry-run default; apply gated). Weekly growth fails closed when site/token/endpoint are unavailable.
...
6. **Restore or retire `qa:funnel-reconcile` CLI** — **Done (#184):** `scripts/funnel-reconcile.mjs` calls `POST /api/analytics/funnel/reconcile` with dry-run default, aggregate-only output, and double-gated `--apply` (`FUNNEL_RECONCILE_ALLOW_APPLY=1`).
...
Historical note: funnel-reconcile emptiness was confirmed on earlier `main` tips (`f8f47ff…`, then `d14e536…`). Restored by #184 — see current `scripts/funnel-reconcile.mjs` and unit coverage in `scripts/unit/funnelReconcile.test.mjs`.

### Inspected (evidence; not all modified)

**Docs:** `PURCHASE_ANALYTICS.md`, `PURCHASE_FUNNEL_AUDIT.md`, `GOAL_10K_2026.md`, `GROWTH_OPS_WEEKLY.md`, `BIG_MOVES_ROADMAP.md`, `LEVERAGE_ROADMAP.md`, `block-1.5-funnel-read.md`, `post-purchase-access-architecture.md`, `PRODUCT_EXECUTION_QUEUE.md`, `OPS_RUNBOOK.md`, `ADS_UTM_REFERENCE.md`, `ADS_RELAUNCH_SETUP.md`, `audits/star-001`…`star-006`, `docs/AGENT_OPERATING_MODEL.md` (repo root), GitHub issues #173 / #179 / #184.
...
**Scripts / package:** `scripts/commerce-digest.mjs`, `revenue-goal-scorecard.mjs`, `loop-scorecard.mjs`, `funnel-reconcile.mjs` (restored), `recovery-email-diag.mjs`, `package.json` script entries.

### Changed in this deliverable (effective PR diff)

- `star-map-app-final/docs/COMMERCIAL_METRIC_DICTIONARY.md` (this file; no-op gap closed for funnel CLI)
- `star-map-app-final/docs/GROWTH_OPS_WEEKLY.md` (truthful funnel-reconcile behavior; no-op caveat removed)
- Related #184 implementation: `scripts/funnel-reconcile.mjs`, `scripts/unit/funnelReconcile.test.mjs`, operator quick reference / block-1.5 env notes
