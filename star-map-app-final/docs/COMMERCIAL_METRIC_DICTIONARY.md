...
| `include_card`                             | no                | boolean — print + card bundle; **only** valid when `order_type=print`, `print_variant=poster_framed`, and `include_digital=false` (checkout semantics). Any other combination is rejected at sanitize.                                                    |
