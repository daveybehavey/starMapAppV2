...
| **Recovery failure**                                    | Recovery email send error or customer cannot regain access                                                                  | Session `recoveryEmailError` / `recoveryEmailErrorCode` (sanitized category only), `recoveryEmailRetryability` (`delivered` \| `retryable` \| `terminal` \| `not_configured`); delivered marker written only after provider success; success/download recovery PostHog events; support paths | `partial`                             | medium for send-state taxonomy; low–medium for regain-access outcomes            |
