...
| **QA exclusion**     | Same markers as `commerceAnalyticsQa.mjs` (`qa_run`, `qa_ops_checkout`, `qa_source` prefixes `live_conversion*` / `live_print_conversion*`). Legacy sessions whose only marker is `client_reference_id=qa-live-conversion` must be classified as QA **before** that identifier is stripped — either exclude the row, set a non-identifying QA flag, or let the CLI derive `qa_source=live_conversion_legacy` from the exact marker (never retain/print the raw client-reference). Excluded counts/revenue reported separately. |
...
3. **Legacy QA before identifier removal:** If a session’s only QA marker is `client_reference_id=qa-live-conversion`, either **exclude the row** from the export or **derive a non-identifying QA flag** (`qa_run` / `qa_ops_checkout` / `qa_source` with a `live_conversion*` prefix) **before** stripping `client_reference_id`. Never leave the raw client-reference value in the sanitized file, logs, or fixtures. (If the exact legacy marker is still present at CLI ingest, the CLI classifies it as QA, sets `qa_source=live_conversion_legacy`, drops the identifier from the in-memory record, and excludes the row — still never printing or persisting the raw value.)
