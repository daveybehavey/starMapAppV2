...
| `include_digital`                          | no                | boolean — print + HD digital bundle. Stripe checkout alias `print_include_digital` (and camel/Pascal forms) is accepted and normalized to this field before unknown-field stripping. Conflicting canonical+alias values fail closed.                                                        |
| `include_card`                             | no                | boolean — print + card bundle; **only** valid when `order_type=print`, `print_variant=poster_framed`, and `include_digital=false` (checkout semantics). Stripe alias `print_include_card` (and camel/Pascal forms) is accepted the same way. Any other combination is rejected at sanitize. |
