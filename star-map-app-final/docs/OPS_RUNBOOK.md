...
- **Checkout recovery email (`checkout.session.expired`):** Resend is the preferred path and uses a deterministic opaque `Idempotency-Key`. Canonical session success is persisted with **`kv.setDurable`** (Cloudflare put failures surface; no silent local-only success) **before** the delivered marker is written. The canonical session is **success-monotonic** and is written **only** on confirmed provider delivery; already-delivered / observed-winner paths acknowledge without rewriting `sessionKey`. Non-delivered outcomes write only a separate sanitized **attempt record** (`stripe:checkout_recovery:attempt:<sessionId>:<attemptId>`, ~45-day TTL). If durable session persistence fails after **Resend** delivery, the marker is not written and the webhook returns **503** for safe idempotent retry. If durable persistence fails after **SendGrid** delivery, the webhook **acknowledges/finalizes (2xx)** without writing the session or delivered marker — preferring no duplicate customer email over complete observability (SendGrid has no provider idempotency; see `SENDGRID_RECOVERY_CONCURRENCY_GUARANTEE` and `isCheckoutRecoveryDurablePersistFailureRetryable`). Retryable failures with no observed delivery return **503** (event dedupe unfinalized); terminal/not-configured acknowledge **2xx**. Concurrent duplicate protection on SendGrid remains best-effort under Workers KV. Diagnostic-only: `scripts/recovery-email-diag.mjs` (no resend execution path).
